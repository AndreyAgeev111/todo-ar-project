...
trait UserTaskRepository[F[_]] {
  ...
  def findUserTaskById(taskId: String): F[Option[UserTaskDto]]
}
...
class UserTaskRepositoryImpl(userTaskDao: UserTaskDao) extends UserTaskRepository[IO] {
  ...
  override def findUserTaskById(taskId: String): IO[Option[UserTaskDto]] =
    userTaskDao.findTaskById(taskId).transact(xa)
}
